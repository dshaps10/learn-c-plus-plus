...
-----------------------------------
The code for randomInteger produces the number in 4 steps:
1) Generate a random real number d in the range [0..1]
2) Scale the numer to the range [0..N] where N is the number of values
3) Translate the number so that the range starts at the appropriate value
4) Convert the result to the next lower integer

The implementation is complicated by that fact that both the expression
	RAND_MAX + 1
...
------------------------------------
The initRandomSeed function decalres a static variable that keeps track
